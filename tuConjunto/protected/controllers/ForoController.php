...
class ForoController extends Controller {
    ...
    public function accessRules() {
        return array(
            ...
            array('allow', // allow authenticated user to perform 'update' actions
                'actions' => array('index', 'view', 'create', 'response'),
                'roles' => array(CrugeAuthitem2::$rolResidente),
            ),
            array('allow', // allow authenticated user to perform 'update' actions
                'actions' => array('index', 'view', 'response'),
    ...
    public function actionCreate() {
        ...
        $model->idAutor = $contacto->idContacto;
        ...
        if (isset($_POST['Foro'])) {
            ...
            if ($noticiasPersistence->crearNoticia($model)) {
                $this->redirect(array('view', 'id' => $model->idForo));
            }
        }
    ...
    public function actionResponse($pregunta) {
        $model = new Foro;
        $contacto = Contactos::model()->find("cruge_user_iduser=:cruge_user_iduser", array(":cruge_user_iduser" => Yii::app()->user->id));
        $model->idAutor = $contacto->idContacto;
        $model->respondeA = $pregunta;
        $model->Conjuntos_idConjunto = $contacto->Conjuntos_idConjunto;
        $model->tema = $pregunta . " - Respuesta";
        // Uncomment the following line if AJAX validation is needed
        // $this->performAjaxValidation($model);        
        $model->fechaPub = date('Y-m-d H:i:s');
        if (isset($_POST['Foro'])) {
            ...
            $noticiasPersistence = new NoticiasPersistence;
            if ($noticiasPersistence->crearNoticia($model)) {
                $this->redirect(array('view', 'id' => $pregunta));
            }
        }
